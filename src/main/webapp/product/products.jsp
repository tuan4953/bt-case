...
<%
    ...
    List<GameAccount> list=(List<GameAccount>)request.getAttribute("list");
    ...
    List<GameAccount> availableList=new ArrayList<>();

    if(list!=null){
        for(GameAccount g:list){
            if(!"SOLD".equalsIgnoreCase(g.getStatus()) && !"DELETED".equalsIgnoreCase(g.getStatus())){
                availableList.add(g);
            }
        }
    }
    ...
    if(start<total){
        pageList=availableList.subList(start,end);
    }
%>
    ...
    <style>
        *, *::before, *::after {
        ...
        .card-product:hover {
            ...
            box-shadow: 0 12px 40px rgba(0,0,0,0.5), 0 0 0 1px rgba(245,197,24,0.15);
        }
        ...
        .product-buttons button:active { transform: scale(0.97); }
        ...
        @media (min-width: 1280px) {
            .container-products { grid-template-columns: repeat(4, 1fr); }
...
<div class="header">
    <div class="header-top">
        <a href="<%= request.getContextPath() %>/home.jsp" class="logo">⚡ GAME STORE</a>
    ...
    <div class="menu">
        ...
        <a class="cart-link" href="<%= request.getContextPath() %>/cart">
            🛒 Giỏ hàng
        ...
        <a href="<%= request.getContextPath() %>/purchase-history">📜 Lịch sử</a>
        <a href="<%= request.getContextPath() %>/user/deposit.jsp">💳 Nạp tiền</a>
        ...
        <a href="<%= request.getContextPath() %>/product/add-product.jsp">➕ Thêm acc</a>
...
<form action="<%= request.getContextPath() %>/products" method="get">
    <div class="search-wrap">
        <div class="search-box">
            <input type="text" name="keyword" placeholder="🔍 Tìm kiếm acc game..." value="<%= keyword %>">
    ...
    <%
        if(pageList!=null && !pageList.isEmpty()){
            ...
            for(GameAccount g:pageList){
                String img=(g.getImage()!=null && !g.getImage().isEmpty()) ? g.getImage() : "default.jpg";
                cardIndex++;
    %>

    <div class="card-product"
         style="animation-delay: <%= (cardIndex * 0.05) %>s"
         data-url="<%= request.getContextPath() %>/account-detail?id=<%= g.getId() %>">

        <div class="card-img-wrap">
            <img src="<%= request.getContextPath() %>/image?name=<%= img %>" alt="<%= g.getGameName() %>" loading="lazy">
        ...
        <div class="product-buttons">
            ...
            <a class="btn-edit" href="<%= request.getContextPath() %>/edit-product?id=<%= g.getId() %>">
                ✏ Sửa
            </a>

            <button class="btn-delete" onclick="openDelete(event,<%= g.getId() %>)">
                🗑 Xoá
            </button>
            ...
            <a class="btn-cart"
               ...
               onclick="openActionModal(event,'<%= request.getContextPath() %>/add-to-cart?id=<%= g.getId() %>','🛒 XÁC NHẬN THÊM GIỎ','Bạn có chắc muốn thêm acc này vào giỏ hàng không?')">
                🛒 Giỏ hàng
            </a>

            <a class="btn-buy"
               href="javascript:void(0)"
               onclick="openActionModal(event,'<%= request.getContextPath() %>/buy-now?id=<%= g.getId() %>','💳 XÁC NHẬN MUA ACC','Bạn có chắc muốn mua acc này không?')">
                💳 Mua ngay
            </a>
...
<div class="pagination-box">
    ...
    <a class="page-btn <%= (i==pageIndex ? "active-page" : "") %>"
       href="<%= request.getContextPath() %>/products?page=<%= i %>&keyword=<%= keyword %>">
        <%= i %>
    </a>
...
<script>
    ...
    function confirmDelete() {
        window.location.href = "<%= request.getContextPath() %>/delete-product?id=" + deleteId;
    }
